...
mcmc_rank_hist(fit$draws(c("TVVC", "TVVMAX", "TVKM", "TVKA")))
mcmc_rank_hist(fit$draws(c("omega_vc", "omega_vmax", "omega_km", "omega_ka")))
...
blah <- draws_df %>%
  gather_draws(c(VC, VMAX, KM, KA)[ID], 
...
est_ind <- draws_df %>%
  spread_draws(c(VC, VMAX, KM, KA, eta_vc, eta_vmax, eta_km, eta_ka)[ID]) %>%
  mean_qi() %>% 
  select(ID, VC, VMAX, KM, KA,
         eta_vc, eta_vmax, eta_km, eta_ka) %>% 
  mutate(ID = factor(ID))
