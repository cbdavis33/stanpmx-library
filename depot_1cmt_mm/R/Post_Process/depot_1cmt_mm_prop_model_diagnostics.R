...
nonmem_data <- read_csv("depot_1cmt_mm/Data/depot_1cmt_mm_prop.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_1cmt_mm/Stan/Fits/depot_1cmt_mm_prop.rds")
...
model <- cmdstan_model(
  "depot_1cmt_mm/Stan/Predict/depot_1cmt_mm_prop_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_mm_prop_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_1cmt_mm_prop_npde.R for an alternate but 
...
blah <- draws_df %>%
  gather_draws(VC[ID], VMAX[ID], KM[ID], KA[ID], TVVC, TVVMAX, TVKM, TVKA) %>%
  ungroup() %>%
