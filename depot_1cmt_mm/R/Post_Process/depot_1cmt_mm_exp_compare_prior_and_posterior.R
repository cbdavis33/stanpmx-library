...
stan_data <- jsonlite::read_json(
  "depot_1cmt_mm/Stan/Fits/Stan_Data/exp.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$prior_only <- stan_data$no_gq_predictions <- 1

model <- cmdstan_model("depot_1cmt_mm/Stan/Fit/depot_1cmt_mm_exp.stan",
...
draws_all_df <- priors$draws(format = "draws_df") %>% 
  ...
  pivot_longer(cols = TVVC:sigma, names_to = "variable", values_to = "value")
