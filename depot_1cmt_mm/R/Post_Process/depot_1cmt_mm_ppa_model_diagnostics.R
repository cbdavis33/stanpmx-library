...
nonmem_data <- read_csv("depot_1cmt_mm/Data/depot_1cmt_mm_ppa.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "depot_1cmt_mm/Stan/Predict/depot_1cmt_mm_ppa_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_mm_ppa_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_1cmt_mm_ppa_npde.R for an alternate but 
