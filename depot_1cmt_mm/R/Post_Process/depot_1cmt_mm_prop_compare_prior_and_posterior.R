...
stan_data <- jsonlite::read_json(
  "depot_1cmt_mm/Stan/Fits/Stan_Data/prop.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$prior_only <- stan_data$no_gq_predictions <- 1

model <- cmdstan_model("depot_1cmt_mm/Stan/Fit/depot_1cmt_mm_prop.stan",
...
draws_all_df <- priors$draws(format = "draws_df") %>% 
  ...
  pivot_longer(cols = TVVC:sigma_p, names_to = "variable", values_to = "value")

(target_comparison_tv <- draws_all_df %>% 
    filter(str_detect(variable, "TV")) %>%
    mutate(variable = factor(variable, 
                             levels = str_c("TV", c("VC", "VMAX", "KM", "KA")))) %>% 
    ggplot() +
...
(target_comparison_omega <- draws_all_df %>% 
    ...
    mutate(variable = factor(variable, 
                             levels = str_c("omega_", c("vc", "vmax", "km", "ka"))),
           variable = fct_recode(variable, "omega[VC]" = "omega_vc",
...
(target_comparison_cor <- draws_all_df %>% 
    ...
    mutate(variable = factor(variable, 
                             levels = c("cor_vc_vmax", "cor_vc_km", "cor_vc_ka",
