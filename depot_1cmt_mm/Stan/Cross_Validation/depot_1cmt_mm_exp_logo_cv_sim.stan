...
generated quantities{
 ...
 
  {
    ...
    vector[n_random] typical_values = to_vector({TVVC, TVVMAX, TVKM, TVKA});
    ...
    for(m in 1:n_sim){
      ...
      for(j in 1:n_subjects){
        ...
        real VC_new = theta_new[1];   // * weight_adjustment_cl;
        real VMAX_new = theta_new[2]; // * weight_adjustment_vc;
        real KM_new = theta_new[3];
        real KA_new = theta_new[4];
      ...
      for(i in 1:n_obs){
        if(bloq_obs[i] == 1){
          log_lik_subj_sim[dv_obs_id[i], m] += 
            lognormal_lcdf(lloq_obs[i] | log(epred_stan[i]), sigma);
        }else{
          log_lik_subj_sim[dv_obs_id[i], m] += 
            lognormal_lpdf(dv_obs[i] | log(epred_stan[i]), sigma);
        }
      }
    }
