...
filename_base <- "transit_fixed_ntr_2cmt_linear/Stan/Fits"

fit_ppa <- read_rds(file.path(filename_base, "transit_fixed_ntr_2cmt_ppa.rds"))
...
stan_data_ppa_no_delay <- jsonlite::read_json(
  "transit_fixed_ntr_2cmt_linear/Stan/Fits/Stan_Data/ppa_no_delay.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data_ppa_zero_into_depot <- jsonlite::read_json(
  "transit_fixed_ntr_2cmt_linear/Stan/Fits/Stan_Data/ppa_zero_into_depot.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data_ppa$n_sim <- stan_data_ppa_no_delay$n_sim <- stan_data_ppa_zero_into_depot$n_sim <- 1000

model_ppa <- cmdstan_model(
  "transit_fixed_ntr_2cmt_linear/Stan/Cross_Validation/transit_fixed_ntr_2cmt_ppa_logo_cv_sim.stan")
