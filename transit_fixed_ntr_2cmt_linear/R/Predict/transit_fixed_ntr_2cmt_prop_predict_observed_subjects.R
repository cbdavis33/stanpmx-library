...
new_data_to_simulate <- nonmem_data %>% 
  ...
  group_by(ID) %>% 
  slice(c(1, n())) %>% 
  expand(time = sort(unique(c(seq(0.1, 2, by = 0.1), 
                              seq(time[1], time[2], by = 0.5))))) %>%
...
model <- cmdstan_model(
  "transit_fixed_ntr_2cmt_linear/Stan/Predict/transit_fixed_ntr_2cmt_prop_predict_observed_subjects.stan")
...
for(i in 1:ggforce::n_pages(tmp)){
  print(ggplot() +
          ...
          scale_y_continuous(name = latex2exp::TeX("$Drug\\;Conc.\\;(\\mu g/mL)$"),
                             ...
                             trans = "identity") +
          scale_x_continuous(name = "Time (h)",
                             breaks = seq(0, max(nonmem_data$time), by = 2),
                             labels = seq(0, max(nonmem_data$time), by = 2),
                             limits = c(143, 150)) +
          theme_bw() +
          theme(axis.text = element_text(size = 14, face = "bold"),
                axis.title = element_text(size = 18, face = "bold"),
