...
fit <- read_rds("transit_fixed_ntr_2cmt_linear/Stan/Fits/transit_fixed_ntr_2cmt_exp.rds")
...
nonmem_data <- read_csv("transit_fixed_ntr_2cmt_linear/Data/transit_fixed_ntr_2cmt_exp.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
# For this example, let's simulate 100 mg, 200 mg, 400 mg, 800 mg, 1200 mg, and 
#   1800 mg for a week
dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 1, 
                                   amt = c(100, 200, 400, 800, 1200, 1800), 
                                   tinf = 0, evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
  realize_addl() %>% 
...
model <- cmdstan_model(
  "transit_fixed_ntr_2cmt_linear/Stan/Predict/transit_fixed_ntr_2cmt_exp_predict_new_subjects.stan")
...
regimens <- str_c(c(100, 200, 400, 800, 1200, 1800), " mg")
...
est_ind <- if(stan_data$want_auc_cmax){
  preds_df %>%
    ...
    median_qi() %>% 
    select(ID, CL, VC, Q, VP, KA, MTT, KTR,
  ...
  preds_df %>%
    spread_draws(c(CL, VC, Q, VP, KA, MTT, KTR, t_half_alpha, t_half_terminal)[ID]) %>% 
    median_qi() %>% 
    select(ID, CL, VC, Q, VP, KA, MTT, KTR, t_half_alpha, t_half_terminal) %>% 
    inner_join(post_preds_summary %>% 
                 filter(time == 168) %>% 
                 select(ID, c_trough = "epred") %>% 
                 distinct(),
