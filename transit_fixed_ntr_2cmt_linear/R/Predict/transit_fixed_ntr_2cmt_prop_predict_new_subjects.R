...
fit <- read_rds("transit_fixed_ntr_2cmt_linear/Stan/Fits/transit_fixed_ntr_2cmt_prop.rds")

stan_data_fit <- jsonlite::read_json(
  "transit_fixed_ntr_2cmt_linear/Stan/Fits/Stan_Data/prop.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

nonmem_data <- read_csv("transit_fixed_ntr_2cmt_linear/Data/transit_fixed_ntr_2cmt_prop.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
