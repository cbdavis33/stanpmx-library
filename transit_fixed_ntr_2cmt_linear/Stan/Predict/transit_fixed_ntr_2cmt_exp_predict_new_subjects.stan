...
generated quantities{
 ...
 
  {
    ...
    for(j in 1:n_subjects_new){
      ...
      if(want_auc_cmax == 1){
        ...
        auc[subj_start[j]:subj_end[j]] = 
                    x_epred[subj_start[j]:subj_end[j], (n_transit + 5)] ./ VC[j];
                    
        auc_ss[j] = max(x_epred[subj_start[j]:subj_end[j], n_transit + 6]) / VC[j];
        c_max[j] = max(x_epred[subj_start[j]:subj_end[j], n_transit + 7]);
        t_max[j] = max(x_epred[subj_start[j]:subj_end[j], n_transit + 8]) - t_1;
    ...
    vector[n_subjects_new] alpha = 0.5*(CL./VC + Q./VC + Q./VP + 
                            sqrt((CL./VC + Q./VC + Q./VP)^2 - 4*CL./VC.*Q./VP));
    vector[n_subjects_new] beta = 0.5*(CL./VC + Q./VC + Q./VP - 
                            sqrt((CL./VC + Q./VC + Q./VP)^2 - 4*CL./VC.*Q./VP));
