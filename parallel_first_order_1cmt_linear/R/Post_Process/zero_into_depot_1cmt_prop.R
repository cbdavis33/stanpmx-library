...
nonmem_data <- read_csv(
  "zero_into_depot_1cmt_linear/Data/zero_into_depot_1cmt_prop.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds(
  "zero_into_depot_1cmt_linear/Stan/Fits/zero_into_depot_1cmt_prop.rds")
...
blah <- draws_df %>%
  gather_draws(CL[ID], VC[ID], KA[ID], DUR[ID], TVCL, TVVC, TVKA, TVDUR) %>%
  ungroup() %>%
...
plot_shrinkage <- function(.variable = c("CL", "VC", "KA", "DUR"),
...
(shrinkage <- draws_df %>%
    ...
    left_join(draws_df %>%
                gather_draws(omega_cl, omega_vc, omega_ka, omega_dur) %>% 
                ungroup() %>% 
                arrange(.draw) %>% 
                rename(omega = ".value") %>% 
...
data_shrinkage_by_draw <- draws_for_shrinkage %>%
  ...
  inner_join(draws_for_shrinkage %>%
               gather_draws(CL[ID], VC[ID], KA[ID], DUR[ID]) %>%
               ungroup()  %>% 
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], KA[ID], DUR[ID],
               eta_cl[ID], eta_vc[ID], eta_ka[ID], eta_dur[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, KA, DUR,
         eta_cl, eta_vc, eta_ka, eta_dur) %>% 
  mutate(ID = factor(ID))
