...
stopifnot(n_depots_with_delay %in% c(n_depots, n_depots - 1))
...
stan_data <- list(n_subjects = n_subjects,
                  ...
                  length_tvdur_prior = 1, # = 1 or n_depots_with_delay
                  scale_tvka = c(0.01, 0.01, 0.1), # must have length 'length_tvka_prior'
                  scale_tvdur = 1000,               # must have length 'length_tvdur_prior'
                  scale_tvcl = 1,
                  scale_tvvc = 1,
...
model <- cmdstan_model(
  file.path("parallel_first_order_1cmt_linear",
            ...
            "Stan", 
            "Fit",
            "parallel_first_order_1cmt_ppa.stan"),
...
fit <- model$sample(
  ...
  refresh = 500,
  max_treedepth = 10,
  output_dir = "parallel_first_order_1cmt_linear/zero_into_depot_for_delays/Stan/Fits/Output",
  output_basename = str_c("parallel_", n_depots, "_first_order_1cmt_",
  ...
  init = function() list(TVKA = sort(rlnorm(n_depots, log(0.01), 1)),
                         ...
                         omega = rlnorm(n_depots + n_depots_with_delay + 2, 
                                        log(0.3), 0.3),
                         sigma = rlnorm(2, c(log(0.2), log(0.002)), 0.3)))
...
fit$save_data_file(dir = file.path("parallel_first_order_1cmt_linear",
                                   "zero_into_depot_for_delays",
                                   "Stan", 
                                   "Fits",
                                   "Stan_Data"),
                   basename = str_c("parallel_", n_depots, "_first_order_1cmt_",
