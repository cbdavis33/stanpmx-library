...
fit_mat_exp <- model$sample(
  ...
  refresh = 100,
  max_treedepth = 10,
  init = function() list(TVKA = sort(rlnorm(n_depots, log(0.1), 1)),
...
fit_mat_exp$save_object(
  file.path("parallel_first_order_1cmt_linear",
            ...
            "Stan", 
            "Fits",
            "parallel_first_order_1cmt_no_initial_delay_prop_mat_exp.stan"))
...
fit_rk45 <- model$sample(
  ...
  init = function() list(TVKA = sort(rlnorm(n_depots, log(0.1), 1)),
                         TVCL = rlnorm(1, log(1), 0.3),
                         TVVC = rlnorm(1, log(8), 0.3),
                         TVDUR = sort(rlnorm(n_depots - 1, log(500), 0.7), 
                                      decreasing = TRUE),
                         omega = rlnorm(n_depots + (n_depots - 1) + 2, 
                                        log(0.3), 0.3),
                         sigma_p = rlnorm(1, log(0.2), 0.3)))
...
fit_rk45$save_object(
  file.path("parallel_first_order_1cmt_linear",
            ...
            "Stan", 
            "Fits",
            "parallel_first_order_1cmt_no_initial_delay_prop_rk45.stan"))
...
fit_bdf$save_object(
  file.path("parallel_first_order_1cmt_linear",
            ...
            "Stan", 
            "Fits",
            "parallel_first_order_1cmt_no_initial_delay_prop_bdf.stan"))
