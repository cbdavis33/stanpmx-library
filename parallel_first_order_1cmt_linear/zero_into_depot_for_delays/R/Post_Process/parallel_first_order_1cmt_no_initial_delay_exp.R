...
## Read in fit
fit <- read_rds(file.path(
  "parallel_first_order_1cmt_linear",
  "zero_into_depot_for_delays",
  "Stan", 
  "Fits",
  "parallel_first_order_1cmt_no_initial_delay_exp.rds"))
...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVKA", "TVDUR")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_dur")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("sigma")),
           combo = c("dens_overlay", "trace"))

mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVKA", "TVDUR")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_dur")))
...
ggplot() +
  geom_density(data = blah %>% 
                 ...
                 mutate(.variable = blah %>% 
                          filter(is.na(ID)) %>% 
                          mutate(.variable = 
                                   case_when(!is.na(Depot) ~ 
                                               str_c(.variable, "_", Depot),
                                             TRUE ~ .variable)) %>% 
                          pull(.variable) %>% 
                          str_remove("TV"),
                        ID = "Population"),
  ...
  scale_fill_discrete(name = "Population Parameter") +
  guides(color = "none") +
...
(shrinkage <- draws_df %>%
    ...
    left_join(draws_df %>%
                gather_draws(omega_cl, omega_vc) %>% 
                ungroup() %>% 
                bind_rows(draws_df %>% 
                            gather_draws(omega_ka[Depot], omega_dur[Depot]) %>%
                            ungroup() %>% 
                            mutate(.variable = 
                                     case_when(!is.na(Depot) ~ 
                                                 str_c(.variable, "_", Depot),
               ...
               bind_rows(draws_for_shrinkage %>%
                           ...
                           mutate(.variable = 
                                    case_when(!is.na(Depot) ~ 
                                                str_c(.variable, "_", Depot),
                                              TRUE ~ .variable),
                                  .variable = str_remove(.variable, "eta_") %>%
                                    toupper()) %>% 
                           select(-Depot) %>% 
                           rename(ind_params = `.value`) %>%
                           select(-ID) %>%
              ...
              bind_rows(draws_df %>%
                          ...
                          mutate(.variable = 
                                   case_when(!is.na(Depot) ~ 
                                               str_c(.variable, "_", Depot),
                                             TRUE ~ .variable),
                                 .variable = str_remove(.variable, "omega_")) %>% 
                          summarize(estimate = mean(.value)) %>% 
                          ungroup() %>% 
                          select(-Depot)),
            by = ".variable") %>% 
...
est_ind <- draws_df %>%
  ...
  inner_join(draws_df %>%
             ...
             by = "ID") %>% 
  select(!contains(".")) %>% 
  select(ID, CL, VC, starts_with("KA"), starts_with("DUR"), everything())
