...
new_data_to_simulate <- nonmem_data %>% 
  ...
  group_by(ID) %>% 
  slice(c(1, n())) %>% 
  expand(time = c(seq(time[1], 23.5, by = 1), seq(24, time[2], by = 96))) %>%
...
model <- cmdstan_model(
  file.path("parallel_first_order_1cmt_linear",
            ...
            "Stan", 
            "Predict",
            "parallel_first_order_1cmt_prop_predict_observed_subjects.stan"))
...
# Note that this t_half is calculated by log(2)/KE, which doesn't work
# correctly for this model with KA < KE. Be careful here
est_ind <- preds_df %>%
