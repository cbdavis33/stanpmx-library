...
file_path <- if_else(isTRUE(locf), 
                     "iv_2cmt_linear_covariates_time_varying/Generic/Data/iv_2cmt_ppa_covariates_time_varying_generic_locf.csv",
                     "iv_2cmt_linear_covariates_time_varying/Generic/Data/iv_2cmt_ppa_covariates_time_varying_generic_nocb.csv")


fit <- if(isTRUE(locf)){
  read_rds("iv_2cmt_linear_covariates_time_varying/Generic/Stan/Fits/ppa_locf.rds")
}else{
  read_rds("iv_2cmt_linear_covariates_time_varying/Generic/Stan/Fits/ppa_nocb.rds")
}
...
# Q2W and 800 mg Q4W. These covariates are constant, as if we can't predict how 
# they change over time. You could give time-varying covariates if you wanted
...
dosing_data <- dosing_data_q2w %>% 
  ...
  mutate(cmt = 2) # analytical => dose into 2nd compartment
...
model <- cmdstan_model(
  "iv_2cmt_linear_covariates_time_varying/Generic/Stan/Predict/iv_2cmt_ppa_predict_new_subjects_covariates_time_varying.stan")
