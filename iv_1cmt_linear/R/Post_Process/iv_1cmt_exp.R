...
nonmem_data <- read_csv("iv_1cmt_linear/Data/iv_1cmt_exp.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
mcmc_rank_hist(fit$draws(c("TVCL", "TVVC")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc")))
...
plot_shrinkage <- function(.variable = c("CL", "VC"),
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], eta_cl[ID], eta_vc[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, eta_cl, eta_vc) %>% 
  mutate(ID = factor(ID))
