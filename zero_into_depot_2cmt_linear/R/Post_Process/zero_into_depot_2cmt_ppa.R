...
nonmem_data <- read_csv("zero_into_depot_2cmt_linear/Data/zero_into_depot_2cmt_ppa.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
blah <- draws_df %>%
  gather_draws(c(CL, VC, Q, VP, KA, DUR)[ID], 
               TVCL, TVVC, Q, VP, TVKA, TVDUR) %>%
  ungroup() %>%
...
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, Q, VP, KA, DUR,
                 eta_cl, eta_vc, eta_q, eta_vp, eta_ka, eta_dur)[ID]) %>% 
  mean_qi() %>% 
