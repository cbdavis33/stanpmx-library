...
## Read in fit
fit <- read_rds("zero_into_depot_2cmt_linear/Stan/Fits/zero_into_depot_2cmt_ppa.rds")
...
model <- cmdstan_model(
  "zero_into_depot_2cmt_linear/Stan/Predict/zero_into_depot_2cmt_ppa_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/zero_into_depot_2cmt_ppa_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/zero_into_depot_2cmt_ppa_npde.R for an alternate but 
...
ggplot() +
  ...
  scale_fill_manual(name = "Population Parameter",
                    values = c("red", "blue", "green", "magenta", "orange", 
                               "purple")) +
  guides(color = "none") +
