...
fit <- read_rds(
  "zero_into_depot_2cmt_linear/Stan/Fits/zero_into_depot_2cmt_ppa.rds")

nonmem_data <- read_csv(
  "zero_into_depot_2cmt_linear/Data/zero_into_depot_2cmt_ppa.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 1, 
                                   amt = c(50, 100, 200, 400, 600, 800, 1600), 
                                   tinf = 0, evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "zero_into_depot_2cmt_linear/Stan/Predict/zero_into_depot_2cmt_ppa_predict_new_subjects.stan")
...
regimens <- str_c(c(50, 100, 200, 400, 600, 800, 1600), " mg")
...
est_ind <- if(stan_data$want_auc_cmax){
  preds_df %>%
    spread_draws(c(CL, VC, Q, VP, KA, DUR, t_half_alpha, t_half_terminal, 
                   auc_ss, c_max, t_max)[ID]) %>%
    median_qi() %>% 
    select(ID, CL, VC, Q, VP, KA, DUR, t_half_alpha, t_half_terminal, 
           auc_ss, c_max, t_max) %>% 
