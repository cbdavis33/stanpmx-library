...
generated quantities{
 ...
 
  {
    ...
    matrix[n_time_new, want_auc_cmax ? 7 : 3] x_ipred;
    ...
    for(j in 1:n_subjects){
      ...
      VP[j] = theta_j[4];
      KA[j] = theta_j[5];
      BIOAV[j] = Phi(inv_Phi(TVBIOAV) + eta[j, 6]); // Phi(inv_Phi(TVBIOAV) + covariate_effects + eta[j, 6]);
      ...
      if(want_auc_cmax == 1){
        ...
        x_ipred[subj_start[j]:subj_end[j],] =
          pmx_solve_rk45(depot_2cmt_ode,
                         ...
                         {BIOAV_new[j], 1, 1, 1, 1, 1, 1}, tlag, x_r)';
