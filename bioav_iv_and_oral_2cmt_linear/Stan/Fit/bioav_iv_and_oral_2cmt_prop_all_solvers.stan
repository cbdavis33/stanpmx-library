...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
                        ...
                        array[] real tlag, int n_cmt, int solver){
  ...
  if(no_gq_predictions == 0){
    ...
    for(j in 1:n_subjects){
      ...
      if(solver == 1){
        ...
        K_epred[2, 2] = -((CL_new[j] + Q_new[j])/VC_new[j]);
        K_epred[2, 3] = Q_new[j]/VP_new[j];
        K_epred[3, 2] = Q_new[j]/VC_new[j];
        K_epred[3, 3] = -Q_new[j]/VP_new[j];
        ...
        x_epred[subj_start[j]:subj_end[j], ] =
          pmx_solve_linode(time[subj_start[j]:subj_end[j]],
                           ...
                           addl[subj_start[j]:subj_end[j]],
                           ss[subj_start[j]:subj_end[j]],
                           K_epred, {BIOAV_new[j], 1, 1}, tlag)';
                           
        K_p[1, 1] = -ka_p;
        K_p[2, 1] = ka_p;
        K_p[2, 2] = -((cl_p + q_p)/vc_p);
        K_p[2, 3] = q_p/vp_p;
        K_p[3, 2] = q_p/vc_p;
        K_p[3, 3] = -q_p/vp_p;
        ...
        x_epred[subj_start[j]:subj_end[j],] =
          pmx_solve_rk45(depot_2cmt_ode,
                         ...
                         addl[subj_start[j]:subj_end[j]],
                         ss[subj_start[j]:subj_end[j]],
                         {CL_new[j], VC_new[j], Q_new[j], VP_new[j], KA_new[j]}, 
                         {BIOAV_new[j], 1, 1}, tlag)';
