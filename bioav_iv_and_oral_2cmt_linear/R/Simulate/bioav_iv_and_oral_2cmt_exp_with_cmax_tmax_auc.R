...
R[2, 6] <- R[6, 2] <- 0.2 # Put in some correlation between VC and F
...
model <- cmdstan_model(
  "bioav_iv_and_oral_2cmt_linear/Stan/Simulate/bioav_iv_and_oral_2cmt_exp_with_cmax_tmax_auc.stan") 
...
params_ind <- if(stan_data$solver == 4){
  ...
  simulated_data$draws(c("CL", "VC", "Q", "VP", "KA", "BIOAV",
                         "auc_t1_t2", "c_max", "t_max", 
                         "t_half_alpha", "t_half_terminal")) %>% 
    spread_draws(c(CL, VC, Q, VP, KA, BIOAV,
                   auc_t1_t2, c_max, t_max, 
    ...
    left_join(dosing_data %>% 
              ...
              by = "ID") %>% 
    ungroup() %>%
    select(ID, CL, VC, Q, VP, KA, BIOAV, auc_t1_t2, c_max, t_max, 
           t_half_alpha, t_half_terminal, TYPE, COHORT)
...
data %>%
  select(-IPRED) %>%
  write_csv("bioav_iv_and_oral_1cmt_linear/Data/bioav_iv_and_oral_1cmt_exp.csv",
            na = ".")

params_ind %>%
  write_csv("bioav_iv_and_oral_1cmt_linear/Data/bioav_iv_and_oral_1cmt_exp_params_ind.csv",
            na = ".")
