...
nonmem_data <- read_csv(
  "bioav_iv_and_oral_2cmt_linear/Data/bioav_iv_and_oral_2cmt_prop.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("bioav_iv_and_oral_2cmt_linear/Stan/Fits/bioav_iv_and_oral_2cmt_prop.rds")
...
model <- cmdstan_model(
  "bioav_iv_and_oral_2cmt_linear/Stan/Predict/bioav_iv_and_oral_2cmt_prop_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/bioav_iv_and_oral_2cmt_prop_vpc.R for an alternate but equivalent
## way to do VPCs
...
preds_col <- draws_df %>% 
  ...
  bind_cols(nonmem_data %>% 
              filter(evid == 0) %>% 
              select(ID, time, evid, cohort)) %>% 
  filter(evid == 0) %>% 
  select(ID, time, cohort, epred_mean, -i)
...
## NPDEs. Also see ../NPDE/bioav_iv_and_oral_2cmt_prop_npde.R for an alternate but 
