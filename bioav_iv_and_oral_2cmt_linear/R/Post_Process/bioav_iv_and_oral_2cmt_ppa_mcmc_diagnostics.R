...
nonmem_data <- read_csv(
  "bioav_iv_and_oral_2cmt_linear/Data/bioav_iv_and_oral_2cmt_ppa.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
# fileConn <- file("bioav_iv_and_oral_2cmt_linear/output.txt")
...
mcmc_pairs(fit$draws(c("omega_cl", "CL[1]", "omega_vc", "VC[1]",
                       "omega_ka", "KA[1]", "omega_bioav", "BIOAV[1]")),
           transform = list(omega_cl = "log",
                            omega_vc = "log",
                            omega_ka = "log",
                            omega_bioav = "log"),
