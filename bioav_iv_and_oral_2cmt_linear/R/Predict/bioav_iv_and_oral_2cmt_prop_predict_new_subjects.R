...
fit <- read_rds(
  "bioav_iv_and_oral_2cmt_linear/Stan/Fits/bioav_iv_and_oral_2cmt_prop.rds")
...
model <- cmdstan_model(
  "bioav_iv_and_oral_2cmt_linear/Stan/Predict/bioav_iv_and_oral_2cmt_prop_predict_new_subjects.stan")
...
## Individual estimates (posterior median). Note - for the IV subjects, Cmax and
...
est_ind <- if(stan_data$want_auc_cmax){
  ...
  preds_df %>%
    ...
    median_qi() %>% 
    select(ID, CL, VC, Q, VP, KA, BIOAV, t_half_alpha, t_half_terminal) %>% 
    inner_join(post_preds_summary %>% 
                 filter(time == 168) %>% 
                 select(ID, c_trough = "epred_stan", type) %>% 
                 distinct(),
