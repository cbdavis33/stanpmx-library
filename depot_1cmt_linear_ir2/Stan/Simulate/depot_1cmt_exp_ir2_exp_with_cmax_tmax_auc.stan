...
//   fixed to be 1
// exponential error for PK - DV = IPRED*exp(eps_pk)
...
//   a dosing interval)
// PD output can include individual Rmax (response increases to a max), and Tmax
...
functions{
  ...
  vector depot_1cmt_ir2_with_auc_cmax_ode(real t, vector y, array[] real params, 
                                          array[] real x_r, array[] int x_i){
...
generated quantities{
  ...
  vector[want_auc_cmax ? n_subjects : 0] r_max;       // Maximum response
  vector[want_auc_cmax ? n_subjects : 0] t_max_pd;    // Tmax for PD
      ...
      if(coupled){
        ...
        if(want_auc_cmax){
          if(solver == 1){
            ...
            x_ipred[subj_start[j]:subj_end[j],] =
              pmx_solve_rk45(depot_1cmt_ir2_with_auc_cmax_ode,
                             n_cmt,
                             time[subj_start[j]:subj_end[j]],
                             amt[subj_start[j]:subj_end[j]],
                             rate[subj_start[j]:subj_end[j]],
            ...
            x_ipred[subj_start[j]:subj_end[j],] =
              pmx_solve_bdf(depot_1cmt_ir2_with_auc_cmax_ode,
                             n_cmt,
                             time[subj_start[j]:subj_end[j]],
                             amt[subj_start[j]:subj_end[j]],
                             rate[subj_start[j]:subj_end[j]],
            ...
            x_ipred[subj_start[j]:subj_end[j],] =
              pmx_solve_adams(depot_1cmt_ir2_with_auc_cmax_ode,
                              n_cmt,
                              time[subj_start[j]:subj_end[j]],
                              amt[subj_start[j]:subj_end[j]],
                              rate[subj_start[j]:subj_end[j]],
