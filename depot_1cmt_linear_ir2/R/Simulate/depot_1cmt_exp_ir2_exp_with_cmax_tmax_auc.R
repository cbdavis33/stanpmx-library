...
model <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Simulate/depot_1cmt_exp_ir2_exp_with_cmax_tmax_auc.stan")
...
params_ind <- if(stan_data$want_auc_cmax){
  ...
  simulated_data$draws(c("CL", "VC", "KA",
                         "KIN", "KOUT", "IC50", "t_half")) %>% 
    spread_draws(c(CL, VC, KA, KIN, KOUT, IC50, t_half)[i]) %>% 
    inner_join(dosing_data %>% 
                 mutate(i = 1:n()),
               by = "i") %>% 
    ungroup() %>%
    select(ID, CL, VC, KA, KIN, KOUT, IC50, t_half)
...
data %>%
  select(-IPRED) %>%
  write_csv(str_c("depot_1cmt_linear_ir2/Data/depot_1cmt_exp_ir2_exp.csv"),
            na = ".")

params_ind %>%
  write_csv(str_c("depot_1cmt_linear_ir2/Data/depot_1cmt_exp_ir2_exp_params_ind.csv"),
            na = ".")
