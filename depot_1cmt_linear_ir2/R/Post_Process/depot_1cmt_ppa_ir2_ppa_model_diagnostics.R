...
nonmem_data <- read_csv(
  "depot_1cmt_linear_ir2/Data/depot_1cmt_ppa_ir2_ppa.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_1cmt_linear_ir2/Stan/Fits/depot_1cmt_ppa_ir2_ppa.rds")
...
model <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Predict/depot_1cmt_ppa_ir2_ppa_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_ppa_ir2_ppa_vpc.R for an alternate but 
...
## NPDEs. Also see ../NPDE/depot_1cmt_ppa_ir2_ppa_npde.R for an alternate but 
