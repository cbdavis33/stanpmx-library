...
nonmem_data <- read_csv(
  "depot_1cmt_linear_ir2/Data/depot_1cmt_prop_ir2_prop.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_1cmt_linear_ir2/Stan/Fits/depot_1cmt_prop_ir2_prop.rds")
...
p_dv_vs_epred_pk <- ggplot(post_preds_summary%>% 
                             filter(cmt == 2), 
...
p_dv_vs_ipred_pk <- ggplot(post_preds_summary %>% 
                             filter(cmt == 2), 
                           aes(x = ipred, y = DV, color = factor(bloq))) +
  geom_point() +
  scale_color_manual(guide = "none",
                     values = c("0" = "black", "1" = "green")) +
  theme_bw() +
...
p_dv_vs_ppc_pk <- ggplot(post_preds_summary %>% 
                            filter(cmt == 2),
...
p_dv_vs_epred_pd <- ggplot(post_preds_summary%>% 
                             filter(cmt == 3), 
                           aes(x = epred, y = DV, color = factor(bloq))) +
  geom_point() +
  scale_color_manual(guide = "none",
...
p_dv_vs_ppc_pd <- ggplot(post_preds_summary %>% 
                           filter(cmt == 3),
                         aes(x = dv_ppc, y = DV, color = factor(bloq))) +
  geom_point() +
  scale_color_manual(guide = "none",
...
tmp <- ggplot(post_preds_summary) +
  ggforce::facet_wrap_paginate(~ID + cmtn, labeller = label_both,
                               nrow = 3, ncol = 4,
...
model <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Predict/depot_1cmt_prop_ir2_prop_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_prop_ir2_prop_vpc.R for an alternate but 
...
## NPDEs. Also see ../NPDE/depot_1cmt_prop_ir2_prop_npde.R for an alternate but 
...
iwres_point %>% 
  ggplot(aes(x = time, y = iwres_point, group = cmt)) +
  theme_bw() +
  geom_point() +
  geom_hline(yintercept = 0, color = "blue", linewidth = 1.5) +
...
iwres_point %>% 
  ggplot(aes(sample = iwres_point, group = cmt)) + 
  geom_qq() +
  geom_abline() +
  theme_bw() +
  facet_wrap(~ cmt)
...
blah_pd <- draws_df %>%
  gather_draws(KIN[ID], KOUT[ID], IC50[ID], TVKIN, TVKOUT, TVIC50) %>%
  ungroup() %>%
  mutate(across(c(ID, .variable), as.factor))
