...
filename_base <- "depot_1cmt_linear_ir2/Stan/Fits"

fit_ppa <- read_rds(file.path(filename_base, "depot_1cmt_ppa_ir2_ppa.rds"))
...
stan_data <- jsonlite::read_json(
  "depot_1cmt_linear_ir2/Stan/Fits/Stan_Data/ppa_ppa.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$n_sim <- 100

model_ppa <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Cross_Validation/depot_1cmt_ppa_ir2_ppa_logo_cv_sim.stan")

model_prop <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Cross_Validation/depot_1cmt_prop_ir2_prop_logo_cv_sim.stan")

model_exp <- cmdstan_model(
  "depot_1cmt_linear_ir2/Stan/Cross_Validation/depot_1cmt_exp_ir2_exp_logo_cv_sim.stan")
