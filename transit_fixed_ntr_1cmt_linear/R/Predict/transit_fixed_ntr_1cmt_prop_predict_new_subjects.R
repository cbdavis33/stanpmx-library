...
fit <- read_rds("transit_fixed_ntr_1cmt_linear/Stan/Fits/transit_fixed_ntr_1cmt_prop.rds")

nonmem_data <- read_csv("transit_fixed_ntr_1cmt_linear/Data/transit_fixed_ntr_1cmt_prop.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "transit_fixed_ntr_1cmt_linear/Stan/Predict/transit_fixed_ntr_1cmt_prop_predict_new_subjects.stan")
