...
stan_data_fit <- jsonlite::read_json(
  "transit_fixed_ntr_1cmt_linear/Stan/Fits/Stan_Data/exp.json") %>% 
...
model <- cmdstan_model(
  "transit_fixed_ntr_1cmt_linear/Stan/Predict/transit_fixed_ntr_1cmt_exp_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/transit_fixed_ntr_1cmt_exp_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/transit_fixed_ntr_1cmt_exp_npde.R for an alternate but 
...
blah <- draws_df %>%
  gather_draws(CL[ID], VC[ID], KA[ID], MTT[ID], TVCL, TVVC, TVKA, TVMTT) %>%
  ungroup() %>%
