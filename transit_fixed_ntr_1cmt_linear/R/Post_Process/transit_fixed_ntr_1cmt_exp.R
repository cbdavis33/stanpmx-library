...
nonmem_data <- read_csv(
  "transit_fixed_ntr_1cmt_linear/Data/transit_fixed_ntr_1cmt_exp.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds(
  "transit_fixed_ntr_1cmt_linear/Stan/Fits/transit_fixed_ntr_1cmt_exp.rds")
...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVKA", "TVMTT")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_mtt")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("sigma")),
           combo = c("dens_overlay", "trace"))

mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVKA", "TVMTT")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_mtt")))
...
for(i in 1:ggforce::n_pages(tmp)){
  print(ggplot() +
          ...
          scale_x_continuous(name = "Time (h)",
                             breaks = seq(0, max(nonmem_data$time), by = 12),
                             labels = seq(0, max(nonmem_data$time), by = 12),
...
blah <- draws_df %>%
  gather_draws(CL[ID], VC[ID], KA[ID], MTT[ID], 
...
plot_shrinkage <- function(.variable = c("CL", "VC", "KA", "MTT"),
...
(shrinkage <- draws_df %>%
    ...
    left_join(draws_df %>%
                gather_draws(omega_cl, omega_vc, omega_ka, omega_mtt) %>% 
                ungroup() %>% 
                arrange(.draw) %>% 
                rename(omega = ".value") %>% 
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], KA[ID], MTT[ID],
               eta_cl[ID], eta_vc[ID], eta_ka[ID], eta_mtt[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, KA, MTT,
         eta_cl, eta_vc, eta_ka, eta_mtt) %>% 
  mutate(ID = factor(ID))
