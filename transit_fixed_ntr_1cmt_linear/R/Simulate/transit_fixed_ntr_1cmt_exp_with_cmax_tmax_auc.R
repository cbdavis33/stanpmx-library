...
model <- cmdstan_model(
  "transit_fixed_ntr_1cmt_linear/Stan/Simulate/transit_fixed_ntr_1cmt_exp_with_cmax_tmax_auc.stan") 
...
params_ind <- if(stan_data$solver == 3){
  ...
  simulated_data$draws(c("CL", "VC", "KA", "MTT", "KTR",
    ...
    left_join(dosing_data %>% 
              ...
              by = "ID") %>% 
    ungroup() %>%
    select(ID, AMT, CL, VC, KA, MTT, KTR, auc_t1_t2, c_max, t_max, t_half)
  
}else{
  
  simulated_data$draws(c("CL", "VC", "KA", "MTT", "KTR", "t_half")) %>% 
    ...
    left_join(dosing_data %>% 
              ...
              by = "ID") %>% 
    ungroup() %>%
    select(ID, AMT, CL, VC, KA, MTT, KTR, t_half)
...
data %>%
  select(-IPRED) %>%
  write_csv("transit_fixed_ntr_1cmt_linear/Data/transit_fixed_ntr_1cmt_exp.csv",
            na = ".")

params_ind %>%
  write_csv("transit_fixed_ntr_1cmt_linear/Data/transit_fixed_ntr_1cmt_exp_params_ind.csv",
            na = ".")
