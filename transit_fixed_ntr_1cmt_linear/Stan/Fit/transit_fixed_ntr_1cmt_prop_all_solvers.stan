...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
                        ...
                        vector CL, vector VC, vector KA, vector KTR,
                        real sigma_p, 
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      if(solver == 1){
        ...
        x_ipred[subj_start[j]:subj_end[j],] =
          pmx_solve_bdf(transit_fixed_1cmt_ode,
                        n_cmt,
                        time[subj_start[j]:subj_end[j]],
                        amt[subj_start[j]:subj_end[j]],
                        rate[subj_start[j]:subj_end[j]],
...
generated quantities{
  ...
  real<lower = 0> omega_mtt = omega[4];
 ...
 
  {
    ...
    omega_cl_mtt = Omega[1, 4];
    omega_vc_ka = Omega[2, 3];
    omega_vc_mtt = Omega[2, 4];
    omega_ka_mtt = Omega[3, 4];
  ...
  if(no_gq_predictions == 0){
    ...
    theta_new = (rep_matrix(to_row_vector({TVCL, TVVC, TVKA, TVMTT}), n_subjects) .* 
                                                                exp(eta_new));
      ...
      if(solver == 1){
        ...
        x_epred[subj_start[j]:subj_end[j],] =
          pmx_solve_rk45(transit_fixed_1cmt_ode,
                         ...
                         addl[subj_start[j]:subj_end[j]],
                         ss[subj_start[j]:subj_end[j]],
                         {CL_new[j], VC_new[j], KA_new[j], KTR_new[j]}, 
                         bioav, tlag, x_r, x_i)';
