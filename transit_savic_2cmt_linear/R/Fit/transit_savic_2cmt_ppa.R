...
nonmem_data <- read_csv("transit_savic_2cmt_linear/Data/transit_savic_2cmt_ppa.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "transit_savic_2cmt_linear/Stan/Fit/transit_savic_2cmt_ppa.stan",
...
fit <- model$sample(
  ...
  refresh = 50,
  max_treedepth = 10,
  output_dir = "transit_savic_2cmt_linear/Stan/Fits/Output",
  output_basename = "ppa",
...
fit$save_object("transit_savic_2cmt_linear/Stan/Fits/transit_savic_2cmt_ppa.rds")

fit$save_data_file(dir = "transit_savic_2cmt_linear/Stan/Fits/Stan_Data",
