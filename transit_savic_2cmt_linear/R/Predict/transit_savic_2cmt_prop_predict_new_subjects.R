...
model <- cmdstan_model(
  "transit_savic_2cmt_linear/Stan/Predict/transit_savic_2cmt_prop_predict_new_subjects.stan")
...
data <- read_csv("transit_savic_2cmt_linear/Data/transit_savic_2cmt_prop.csv", na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
  group_by(ID) %>% 
