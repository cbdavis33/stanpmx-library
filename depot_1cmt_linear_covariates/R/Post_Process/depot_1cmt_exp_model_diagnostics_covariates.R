...
model <- cmdstan_model(
  "depot_1cmt_linear_covariates/Stan/Predict/depot_1cmt_exp_predict_observed_subjects_covariates.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_exp_vpc_covariates.R for an alternate but 
## equivalent way to do VPCs
...
preds_col <- draws_df %>% 
  ...
  bind_cols(nonmem_data %>% 
              filter(evid == 0) %>% 
              select(ID, time, evid, cmppi, race)) %>% 
  filter(evid == 0) %>% 
  select(ID, time, cmppi, race, epred_mean, -i)
...
## NPDEs. Also see ../NPDE/depot_1cmt_exp_npde_covariates.R for an alternate 
## but equivalent way to do NPDEs
