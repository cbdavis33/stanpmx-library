...
nonmem_data <- read_csv(
  "depot_1cmt_linear_ir4/Data/depot_1cmt_ppa_ir4_ppa.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_1cmt_linear_ir4/Stan/Fits/depot_1cmt_ppa_ir4_ppa.rds")
...
model <- cmdstan_model(
  "depot_1cmt_linear_ir4/Stan/Predict/depot_1cmt_ppa_ir4_ppa_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_ppa_ir4_ppa_vpc.R for an alternate but 
...
## NPDEs. Also see ../NPDE/depot_1cmt_ppa_ir4_ppa_npde.R for an alternate but 
