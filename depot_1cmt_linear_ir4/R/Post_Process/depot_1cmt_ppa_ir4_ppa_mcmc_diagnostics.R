...
nonmem_data <- read_csv("depot_1cmt_linear_ir4/Data/depot_1cmt_ppa_ir4_ppa.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
# fileConn <- file("depot_1cmt_linear_ir4/output.txt")
...
mcmc_combo(fit$draws(parameters_to_examine %>% str_subset("omega")),
           combo = c("dens_overlay", "trace", "violin"))
mcmc_combo(fit$draws(c("sigma_p_pk", "sigma_a_pk", "cor_p_a_pk",
                       "sigma_p_pd", "sigma_a_pd", "cor_p_a_pd")),
           combo = c("dens_overlay", "trace", "violin"))
mcmc_combo(fit$draws(parameters_to_examine %>% 
                       str_subset("cor") %>%
                       `[`(-str_detect(., "p_a")) %>% 
                       `[`(-str_detect(., "p_a"))),
           combo = c("dens_overlay", "trace", "violin"))
...
mcmc_rank_hist(fit$draws(c("sigma_p_pk", "sigma_a_pk", "cor_p_a_pk",
                           "sigma_p_pd", "sigma_a_pd", "cor_p_a_pd")))
mcmc_rank_hist(fit$draws(parameters_to_examine %>% 
                           str_subset("cor") %>%
                           `[`(-str_detect(., "p_a")) %>% 
                           `[`(-str_detect(., "p_a"))))
...
mcmc_rank_overlay(fit$draws(c("sigma_p_pk", "sigma_a_pk", "cor_p_a_pk",
                              "sigma_p_pd", "sigma_a_pd", "cor_p_a_pd")))
mcmc_rank_overlay(fit$draws(parameters_to_examine %>% 
                              str_subset("cor") %>%
                              `[`(-str_detect(., "p_a")) %>% 
                              `[`(-str_detect(., "p_a"))))
...
# mcmc_rank_ecdf(fit$draws(c("sigma_p_pk", "sigma_a_pk", "cor_p_a_pk",
#                            "sigma_p_pd", "sigma_a_pd", "cor_p_a_pd")))
# mcmc_rank_ecdf(fit$draws(parameters_to_examine %>%
#                            str_subset("cor") %>%
#                            `[`(-str_detect(., "p_a")) %>% 
#                            `[`(-str_detect(., "p_a"))))
