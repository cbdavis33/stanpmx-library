...
mcmc_dens(fit$draws(c("sigma_p_pk", "sigma_a_pk", "cor_p_a_pk",
...
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, KA, KIN, KOUT, SC50, 
                 eta_cl, eta_vc, eta_ka, eta_kin, eta_kout, eta_sc50, eta_smax)[ID]) %>%
  mean_qi() %>% 
  select(ID, CL, VC, KA, KIN, KOUT, SC50, 
