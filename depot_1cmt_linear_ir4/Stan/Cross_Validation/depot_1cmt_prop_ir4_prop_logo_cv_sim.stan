...
// IIV on KIN, KOUT, SC50 (full covariance matrix) for PD. SMAX and HILL are 
//   fixed to be 1
...
generated quantities{
 ...
 
  {
    ...
    vector[n_random_pd] typical_values_pd = to_vector({TVKIN, TVKOUT, TVSC50, TVSMAX});
    ...
    for(m in 1:n_sim){
      ...
      for(j in 1:n_subjects){
        ...
        real SC50_new = theta_new_pd[3];
        real SMAX_new = theta_new_pd[4];
        ...
        x_epred[subj_start[j]:subj_end[j],] =
          pmx_solve_onecpt_rk45(depot_1cmt_ir4_ode_coupled,
                                ...
                                {CL_new, VC_new, KA_new,
                                 KIN_new, KOUT_new, SC50_new,
                                 SMAX_new, HILL_new, R_0_new},
                                bioav, tlag)';
      ...
      for(i in 1:n_obs){
        ...
        if(cmt[i_obs[i]] == 2 || cmt[i_obs[i]] == 3){
          ...
          real sigma_tmp = cmt[i_obs[i]] == 2 ? epred_tmp*sigma_p_pk : epred_tmp*sigma_p_pd;
          if(bloq_obs[i] == 1){
