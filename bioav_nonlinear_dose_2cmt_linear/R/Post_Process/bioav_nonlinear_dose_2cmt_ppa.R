...
nonmem_data <- read_csv(
  "bioav_nonlinear_dose_2cmt_linear/Data/bioav_nonlinear_dose_2cmt_ppa.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds(
  "bioav_nonlinear_dose_2cmt_linear/Stan/Fits/bioav_nonlinear_dose_2cmt_ppa.rds")
...
ggplot() +
  ...
  scale_fill_manual(name = "Population Parameter",
                    values = c("red", "blue", "green", "orange", "magenta")) +
  guides(color = "none") +
...
data_shrinkage_by_draw <- draws_for_shrinkage %>%
  ...
  inner_join(draws_for_shrinkage %>%
               gather_draws(eta_cl[ID], eta_vc[ID], eta_q[ID], eta_vp[ID], 
                            eta_ka[ID]) %>%
               ungroup()  %>% 
               mutate(.variable = str_remove(.variable, "eta_") %>%
                        toupper()) %>%
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], KA[ID], Q[ID], VP[ID],
...
# Individual point estimates (posterior mean)
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], Q[ID], VP[ID], KA[ID],
               eta_cl[ID], eta_vc[ID], eta_q[ID], eta_vp[ID], eta_ka[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, Q, VP, KA,
         eta_cl, eta_vc, eta_q, eta_vp, eta_ka) %>% 
...
eta_std <- est_ind %>% 
  ...
  left_join(nonmem_data %>% 
              ...
              mutate(ID = factor(ID),
                     dose = factor(dose)), 
            by = "ID") %>% 
  mutate(parameter = factor(parameter, levels = c("CL", "VC", "Q", "VP", "KA")))
