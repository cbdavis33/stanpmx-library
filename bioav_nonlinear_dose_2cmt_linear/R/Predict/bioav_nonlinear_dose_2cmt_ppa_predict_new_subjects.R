...
fit <- read_rds("bioav_nonlinear_dose_2cmt_linear/Stan/Fits/bioav_nonlinear_dose_2cmt_ppa.rds")
...
dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 1, 
                                   amt = c(100, 200, 400, 600, 800, 1200), 
                                   tinf = 0, evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "bioav_nonlinear_dose_2cmt_linear/Stan/Predict/bioav_nonlinear_dose_2cmt_ppa_predict_new_subjects.stan")
...
regimens <- str_c(c(100, 200, 400, 600, 800, 1200), " mg")
