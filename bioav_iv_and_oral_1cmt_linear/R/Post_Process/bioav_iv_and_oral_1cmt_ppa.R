...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVKA", "TVBIOAV")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_bioav")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("sigma_p", "sigma_a", "cor_p_a")),
           combo = c("dens_overlay", "trace"))

mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVKA", "TVBIOAV")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc", "omega_ka", "omega_bioav")))
...
plot_shrinkage <- function(.variable = c("CL", "VC", "KA", "BIOAV"),
                           std_dev, ind_params, ...){
...
(shrinkage <- draws_df %>%
    ...
    left_join(draws_df %>%
                gather_draws(omega_cl, omega_vc, omega_ka, omega_bioav) %>% 
                ungroup() %>% 
                arrange(.draw) %>% 
                rename(omega = ".value") %>% 
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], KA[ID], BIOAV[ID],
               eta_cl[ID], eta_vc[ID], eta_ka[ID], eta_bioav[ID]) %>% 
  mean_qi() %>% 
