...
nonmem_data <- read_csv("depot_2cmt_linear/Data/depot_2cmt_ppa.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "depot_2cmt_linear/Stan/Predict/depot_2cmt_ppa_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_2cmt_ppa_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_2cmt_ppa_npde.R for an alternate but 
