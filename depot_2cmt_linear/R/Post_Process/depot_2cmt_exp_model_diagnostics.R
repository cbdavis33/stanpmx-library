...
nonmem_data <- read_csv("depot_2cmt_linear/Data/depot_2cmt_exp.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_2cmt_linear/Stan/Fits/depot_2cmt_exp.rds")
...
model <- cmdstan_model(
  "depot_2cmt_linear/Stan/Predict/depot_2cmt_exp_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_2cmt_exp_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_2cmt_exp_npde.R for an alternate but 
