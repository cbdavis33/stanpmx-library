...
// Users choice of general ODE solution (with rk45, bdf, or adams) or coupled 
//   ODE solution (with rk45 or bdf) using Torsten
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      if(coupled){
        if(solver == 1){
          ...
          // fatal_error() is more appropriate, but I think there's a bug with
          // CmdStan v2.33.1, and it won't compile (fatal_error() is fine with
          // CmdStan v2.35.0). I'll change this when Torsten is updated,
          reject("The adams solver isn't available for the coupled solver.");
          // fatal_error("The adams solver isn't available for the coupled solver.")
        }
      }else{
        if(solver == 1){
          ...
          x_ipred[subj_start[j]:subj_end[j],] =
            pmx_solve_adams(depot_1cmt_ir3_ode,
                            n_cmt_pk + n_cmt_pd,
                            time[subj_start[j]:subj_end[j]],
                            amt[subj_start[j]:subj_end[j]],
                            rate[subj_start[j]:subj_end[j]],
...
data{
  ...
  int<lower = 1, upper = 3> solver;  // rk45 = 1, bdf = 2, adams = 3 (adams is non-coupled only)
