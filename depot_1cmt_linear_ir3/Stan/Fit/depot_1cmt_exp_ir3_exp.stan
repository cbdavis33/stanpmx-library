...
// Deals with BLOQ values by the M3 method (M3 and M4 are equivalent with the
...
functions{
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    for(i in 1:n_obs_slice){
      ...
      real sigma_tmp = cmt_slice[i] == 2 ? sigma_pk : sigma_pd;
...
data{
  ...
  real<lower = 0> scale_sigma_pk;   // Prior Scale parameter for lognormal error for PK
...
generated quantities{
  ...
  real<lower = 0> sigma_sq_pk = square(sigma_pk);
