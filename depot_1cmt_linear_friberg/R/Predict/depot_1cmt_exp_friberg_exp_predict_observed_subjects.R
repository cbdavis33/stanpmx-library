...
nonmem_data <- read_csv("depot_1cmt_linear_friberg/Data/depot_1cmt_exp_friberg_exp.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "depot_1cmt_linear_friberg/Stan/Predict/depot_1cmt_exp_friberg_exp_predict_observed_subjects.stan")
