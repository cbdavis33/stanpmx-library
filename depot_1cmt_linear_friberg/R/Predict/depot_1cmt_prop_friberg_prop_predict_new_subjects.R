...
fit <- read_rds("depot_1cmt_linear_friberg/Stan/Fits/depot_1cmt_prop_friberg_prop.rds")

nonmem_data <- read_csv(
  "depot_1cmt_linear_friberg/Data/depot_1cmt_prop_friberg_prop.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
# For this example, let's simulate 5 mg. 10 mg, 20 mg, 40 mg, 80 mg
dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 1, 
                                   amt = c(5, 10, 20, 40, 80)*1000, 
                                   tinf = 0, evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "depot_1cmt_linear_friberg/Stan/Predict/depot_1cmt_prop_friberg_prop_predict_new_subjects.stan")
...
regimens <- str_c(c(5, 10, 20, 40, 80), " mg")
...
data <- read_csv("depot_1cmt_linear_friberg/Data/depot_1cmt_prop_friberg_prop.csv", 
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
p_pk_check <- post_preds_summary %>% 
  filter(regimen %in% str_c(c(5, 10), " mg"),
         cmt == "PK",
         time <= 168) %>%
