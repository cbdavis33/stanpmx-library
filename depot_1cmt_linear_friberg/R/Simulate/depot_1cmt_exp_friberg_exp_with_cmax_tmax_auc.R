...
model <- cmdstan_model(
  "depot_1cmt_linear_friberg/Stan/Simulate/depot_1cmt_exp_friberg_exp_with_cmax_tmax_auc.stan") 
...
params_ind <- simulated_data$draws(c("CL", "VC", "KA",
  ...
  select(ID, CL, VC, KA, MTT, CIRC0, GAMMA, ALPHA,
         auc_t1_t2, c_max, t_max, t_half, r_min)
...
data %>%
  select(-IPRED) %>% 
  write_csv("depot_1cmt_linear_friberg/Data/depot_1cmt_exp_friberg_exp.csv", na = ".")

params_ind %>%
  write_csv("depot_1cmt_linear_friberg/Data/depot_1cmt_exp_friberg_exp_params_ind.csv")
