...
model <- cmdstan_model(
  "depot_1cmt_linear_t/Stan/Predict/depot_1cmt_ppa_t_predict_new_subjects.stan")
...
data <- read_csv("depot_1cmt_linear_t/Data/depot_1cmt_ppa_t.csv", na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
  group_by(ID) %>% 
...
est_ind <- if(stan_data$want_auc_cmax){
  ...
  preds_df %>%
    spread_draws(c(CL, VC, KA, t_half)[ID]) %>% 
    median_qi() %>% 
