...
nonmem_data <- read_csv("depot_1cmt_linear_t/Data/depot_1cmt_ppa_t.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "depot_1cmt_linear_t/Stan/Predict/depot_1cmt_ppa_t_predict_observed_subjects.stan")
...
zxc <- draws_df %>%
  spread_draws(c(epred_stan, epred, ipred, dv_ppc)[i]) %>% 
  mutate(DV = nonmem_data$DV[nonmem_data$evid == 0][i],
         bloq = nonmem_data$bloq[nonmem_data$evid == 0][i],
         lloq = nonmem_data$lloq[nonmem_data$evid == 0][i],
         ID = nonmem_data$ID[nonmem_data$evid == 0][i],
...
## VPCs. Also see ../VPC/depot_1cmt_ppa_t_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_1cmt_ppa_t_npde.R for an alternate but 
...
residuals <- draws_df %>%
  ...
  inner_join(draws_df %>% 
               select(nu, .draw),
             by = ".draw") %>% 
  mutate(iwres_adj = iwres*(sqrt(nu/(nu - 2))))
...
## Check for normally distributed errors (this should look bad, since we had
## Student's-t distributed errors)

residuals_for_qq <- residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(9) %>% 
  group_by(.draw) %>%
  nest()

plots_qq_norm <- residuals_for_qq %>% 
  mutate(plot = map2(data, .draw,
                     ~ ggplot(data = .x, aes(sample = iwres)) +
                       geom_qq() +
                       geom_qq_line() +
                       ggtitle(glue::glue("draw = {.y}")) +
                       theme_bw()))

cowplot::plot_grid(plotlist = map(plots_qq_norm$plot,  ~ .))

## Check for Student's-t distributed errors (this should look better)
plots_qq_t <- residuals_for_qq %>%  
  mutate(plot = map2(data, .draw,
                     ~ ggplot(data = .x, aes(sample = iwres)) +
                       geom_qq(distribution = qt, 
                               dparams = list(df = unique(.x$nu))) +
                       geom_qq_line(distribution = qt, 
                                    dparams = list(df = unique(.x$nu))) +
                       ggtitle(glue::glue("draw = {.y}, df = {unique(.x$nu)}")) +
                       theme_bw()))

cowplot::plot_grid(plotlist = map(plots_qq_t$plot,  ~ .))

# Normally distributed errors shouldn't look good
some_residuals_qq_norm <- residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(100) %>% 
...
animate(some_residuals_qq_norm, nframes = 100, width = 384, height = 384, 
        res = 96, dev = "png", type = "cairo")

plots_residual_hist <- residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(9) %>% 
  group_by(.draw) %>%
  nest() %>%  
  mutate(plot = map2(data, .draw,
                     ~ ggplot(data = .x, aes(x = iwres_adj)) +
                       geom_histogram(aes(y = after_stat(density))) +
                       geom_density(color = "blue", linewidth = 1.5) +
                       stat_function(fun = dt, args = list(df = unique(.x$nu)),
                                     color = "red", size = 1) +
                       theme_bw()))

cowplot::plot_grid(plotlist = map(plots_residual_hist$plot,  ~ .))

plots_residual_vs_time <- residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(9) %>% 
  group_by(nu) %>%
  mutate(qn_lower = qt(0.025, df = nu),
         qn_upper = qt(0.975, df = nu)) %>% 
  ungroup() %>% 
  group_by(.draw) %>% 
  nest() %>%  
  mutate(plot = map2(data, .draw,
                     ~ ggplot(data = .x, aes(x = time, y = iwres_adj)) +
                       geom_point() +
                       geom_hline(yintercept = 0, color = "blue", linewidth = 1.5) +
...
cowplot::plot_grid(plotlist = map(plots_residual_vs_time$plot,  ~ .))

plots_residual_vs_ipred <- residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(9) %>% 
  group_by(nu) %>%
  mutate(qn_lower = qt(0.025, df = nu),
  ...
  nest() %>%  
  mutate(plot = map2(data, .draw,
                     ~ ggplot(data = .x, aes(x = ipred, y = iwres_adj)) +
                       geom_point() +
                       geom_hline(yintercept = 0, color = "blue", linewidth = 1.5) +
                       geom_hline(aes(yintercept = qn_lower), linetype = "dashed", color = "blue",
                                  linewidth = 1.25) +
...
cowplot::plot_grid(plotlist = map(plots_residual_vs_ipred$plot,  ~ .))

# |iwres| vs. IPRED
residuals %>% 
  filter(bloq == 0) %>%
  sample_draws(9) %>% 
  mutate(abs_iwres = abs(iwres_adj)) %>% 
