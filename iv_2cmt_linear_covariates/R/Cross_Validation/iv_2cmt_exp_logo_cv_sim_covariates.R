...
stan_data <- jsonlite::read_json(
  "iv_2cmt_linear_covariates/Stan/Fits/Stan_Data/exp_covariates.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$n_sim <- 1000

model <- cmdstan_model(
  "iv_2cmt_linear_covariates/Stan/Cross_Validation/iv_2cmt_exp_logo_cv_sim_covariates.stan")
