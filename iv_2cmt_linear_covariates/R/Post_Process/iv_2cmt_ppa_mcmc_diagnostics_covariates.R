...
# fileConn <- file("iv_2cmt_linear/output.txt")
...
mcmc_combo(fit$draws(parameters_to_examine %>% str_subset("theta")),
           combo = c("dens_overlay", "trace", "violin"))
...
mcmc_rank_hist(fit$draws(parameters_to_examine %>% str_subset("theta")))
...
mcmc_rank_overlay(fit$draws(parameters_to_examine %>% str_subset("theta")))
...
# mcmc_rank_ecdf(fit$draws(parameters_to_examine %>% str_subset("theta")))
...
mcmc_pairs(fit$draws(map(c("theta", "omega"), str_subset, 
                         string = parameters_to_examine) %>% 
                       reduce(union)),
           diag_fun = "dens",
           off_diag_fun = "hex")
