...
post_preds_summary <- draws_df %>%
  ...
  mutate(DV = nonmem_data$DV[nonmem_data$evid == 0][i],
         ...
         age = nonmem_data$age[nonmem_data$evid == 0][i],
         sex = nonmem_data$sexf[nonmem_data$evid == 0][i],
         wt = nonmem_data$wt[nonmem_data$evid == 0][i],
         egfr = nonmem_data$egfr[nonmem_data$evid == 0][i],
         race = nonmem_data$race[nonmem_data$evid == 0][i]) %>% 
  rowwise() %>% 
...
end_inf_data <- nonmem_data %>% 
  ...
  mutate(tinf = amt/rate,
         time = time + tinf) %>% 
  select(-tinf, -sexf, -age, -race, -wt, -egfr)
...
new_data_to_simulate <- nonmem_data %>% 
  ...
  left_join(nonmem_data %>% 
              select(ID, sexf, age, race, wt, egfr) %>% 
              distinct(),
            by = "ID")
...
stan_data_fit <- jsonlite::read_json(
  "iv_2cmt_linear_covariates/Stan/Fits/Stan_Data/prop_covariates_fixed_allometric.json") %>% 
...
stan_data <- list(n_subjects = n_subjects,
                  ...
                  t_2 = 84,
                  want_auc_cmax = 0,
                  theta_cl_wt = stan_data_fit$theta_cl_wt,
                  theta_vc_wt = stan_data_fit$theta_vc_wt,
                  theta_q_wt = stan_data_fit$theta_q_wt,
                  theta_vp_wt = stan_data_fit$theta_vp_wt)

model <- cmdstan_model(
  "iv_2cmt_linear_covariates/Stan/Predict/iv_2cmt_prop_predict_observed_subjects_covariates_fixed_allometric.stan")
...
post_preds_summary <- post_preds %>% 
  mean_qi(epred, ipred, dv, .width = 0.95) %>%
  ...
  left_join(nonmem_data %>% 
              filter(evid == 0) %>%
              distinct(ID, sexf, age, race, wt, sexf, egfr),
            by = "ID") %>% 
...
## VPCs. Also see ../VPC/iv_2cmt_prop_vpc_covariates_fixed_allometric.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/iv_2cmt_prop_npde_covariates_fixed_allometric.R for an alternate but 
...
## I think there are two ways to look at this, both of which have value:
##   1) by draw - this is similar to "Enhanced Method for Diagnosing 
##        Pharmacometric Models: Random Sampling from Conditional Distributions"
##        from the Lixoft group. 
##   2) using posterior summaries, like a posterior mean for eta-hat (analogous 
##        to EBEs) (1 - sd(individual posterior means)/(posterior mean of omega))
...
(p_draws_age <- draws_for_covariate_examination %>% 
...
(p_draws_wt <- draws_for_covariate_examination %>% 
...
(p_draws_egfr <- draws_for_covariate_examination %>% 
    ggplot(aes(x = egfr, y = .value)) +
    geom_point() +
    scale_x_continuous(name = latex2exp::TeX("$eGFR \\; (mL/min/1.73 \\; m^2)$")) +
    scale_y_continuous(name = "Indiv. Effect") +
    theme_bw(18) +
    geom_hline(yintercept = 0, linetype = "dashed") +
    stat_smooth(method = "loess", span = 0.9) +
...
# Now look at it using posterior point estimates (posterior mean)
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, Q, VP,
                 eta_cl, eta_vc, eta_q, eta_vp)[ID]) %>% 
  mean_qi() %>% 
...
(p_point_race <- eta_std %>% 
    ggplot(aes(x = race, y = eta)) + 
    geom_boxplot() +
    scale_x_discrete(name = "Race") +
    scale_y_continuous(name = "Indiv. Effect") +
    theme_bw(18) +
...
(p_point_egfr <- eta_std %>% 
    ggplot(aes(x = egfr, y = eta)) + 
    geom_point() +
    scale_x_continuous(name = latex2exp::TeX("$eGFR \\; (mL/min/1.73 \\; m^2)$")) +
    scale_y_continuous(name = "Indiv. Effect") +
    theme_bw(18) +
