...
nonmem_data <- read_csv(
  "iv_2cmt_linear_covariates/Data/iv_2cmt_ppa_covariates.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "iv_2cmt_linear_covariates/Stan/Predict/iv_2cmt_ppa_predict_observed_subjects_covariates.stan")
...
## VPCs. Also see ../VPC/iv_2cmt_ppa_vpc_covariates.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/iv_2cmt_ppa_npde_covariates.R for an alternate but 
