...
model <- cmdstan_model(
  "depot_2cmt_linear_ir1/Stan/Simulate/depot_2cmt_exp_ir1_exp_with_cmax_tmax_auc.stan")
...
params_ind <- simulated_data$draws(c("CL", "VC", "Q", "VP", "KA",
  ...
  select(ID, CL, VC, Q, VP, KA, KIN, KOUT, IC50,
         auc_t1_t2, c_max, t_max,
         t_half_alpha, t_half_terminal, r_min, t_min)
...
data %>%
  select(-IPRED) %>% 
  write_csv("depot_2cmt_linear_ir1/Data/depot_2cmt_exp_ir1_exp.csv", na = ".")

params_ind %>%
  write_csv("depot_2cmt_linear_ir1/Data/depot_2cmt_exp_ir1_exp_params_ind.csv")
