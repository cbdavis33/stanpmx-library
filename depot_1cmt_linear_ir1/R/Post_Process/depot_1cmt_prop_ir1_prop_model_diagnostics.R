...
nonmem_data <- read_csv(
  "depot_1cmt_linear_ir1/Data/depot_1cmt_prop_ir1_prop.csv",
  na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
## Read in fit
fit <- read_rds("depot_1cmt_linear_ir1/Stan/Fits/depot_1cmt_prop_ir1_prop.rds")
...
model <- cmdstan_model(
  "depot_1cmt_linear_ir1/Stan/Predict/depot_1cmt_prop_ir1_prop_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_prop_ir1_prop_vpc.R for an alternate but 
...
## NPDEs. Also see ../NPDE/depot_1cmt_prop_ir1_prop_npde.R for an alternate but 
