...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVQ1", "TVVP1", "TVQ2", "TVVP2")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_cl", "omega_vc", "omega_q1", "omega_vp1", 
                       "omega_q2", "omega_vp2")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("sigma")),
           combo = c("dens_overlay", "trace"))

mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVQ1", "TVVP1", "TVQ2", "TVVP2")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc", "omega_q1", "omega_vp1", 
                           "omega_q2", "omega_vp2")))
...
blah <- draws_df %>%
  gather_draws(c(CL, VC, Q1, VP1, Q2, VP2)[ID], 
               TVCL, TVVC, TVQ1, TVVP1, TVQ2, TVVP2) %>%
  ungroup() %>%
...
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, Q1, VP1, Q2, VP2,
                 eta_cl, eta_vc, eta_q1, eta_vp1, eta_q2, eta_vp2)[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, Q1, VP1, Q2, VP2,
         eta_cl, eta_vc, eta_q1, eta_vp1, eta_q2, eta_vp2) %>% 
  mutate(ID = factor(ID))
