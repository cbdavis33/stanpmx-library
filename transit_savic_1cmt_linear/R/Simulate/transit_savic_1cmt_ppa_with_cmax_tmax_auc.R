...
model <- cmdstan_model(
  "transit_savic_1cmt_linear/Stan/Simulate/transit_savic_1cmt_ppa_with_cmax_tmax_auc.stan") 
...
params_ind <- simulated_data$draws(c("CL", "VC", "KA", "NTR", "MTT", 
  ...
  inner_join(dosing_data %>%
             ...
             by = "ID") %>% 
  ungroup() %>%
  select(ID, CL, VC, KA, NTR, MTT, auc_t1_t2, c_max, t_max, t_half)
...
data %>%
  select(-IPRED) %>%
  # write_csv("transit_savic_1cmt_linear/Data/transit_savic_1cmt_ppa.csv", na = ".")
write_csv("transit_savic_1cmt_linear/Data/transit_savic_1cmt_prop.csv", na = ".")

params_ind %>%
  # write_csv("transit_savic_1cmt_linear/Data/transit_savic_1cmt_ppa_params_ind.csv")
write_csv("transit_savic_1cmt_linear/Data/transit_savic_1cmt_prop_params_ind.csv")
