...
## Read in fit
fit <- read_rds("transit_savic_1cmt_linear/Stan/Fits/transit_savic_1cmt_ppa.rds")

## Summary of parameter estimates
parameters_to_summarize <- c(str_subset(fit$metadata()$stan_variables, "TV"),
                             str_c("omega_", c("cl", "vc", "ka", "ntr", "mtt")),
...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVKA", "TVNTR", "TVMTT")),
...
mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVKA", "TVNTR", "TVMTT")))
...
blah <- draws_df %>%
  gather_draws(CL[ID], VC[ID], KA[ID], NTR[ID], MTT[ID], 
               TVCL, TVVC, TVKA, TVNTR, TVMTT) %>%
  ungroup() %>%
...
plot_shrinkage <- function(.variable = c("CL", "VC", "KA", "NTR", "MTT"),
...
(shrinkage <- draws_df %>%
    ...
    left_join(draws_df %>%
                gather_draws(omega_cl, omega_vc, omega_ka, omega_ntr, omega_mtt) %>% 
                ungroup() %>% 
                arrange(.draw) %>% 
                rename(omega = ".value") %>% 
...
est_ind <- draws_df %>%
  spread_draws(CL[ID], VC[ID], KA[ID], NTR[ID], MTT[ID],
               eta_cl[ID], eta_vc[ID], eta_ka[ID], eta_ntr[ID], eta_mtt[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, KA, NTR, MTT,
         eta_cl, eta_vc, eta_ka, eta_ntr, eta_mtt) %>% 
  mutate(ID = factor(ID))
