...
model <- cmdstan_model(
  "zero_into_depot_1cmt_linear/Stan/Predict/zero_into_depot_1cmt_ppa_predict_new_subjects.stan")
...
data <- read_csv("zero_into_depot_1cmt_linear/Data/zero_into_depot_1cmt_ppa.csv", na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
  group_by(ID) %>% 
