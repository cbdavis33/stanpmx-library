...
model <- cmdstan_model(
  "zero_into_depot_1cmt_linear/Stan/Predict/zero_into_depot_1cmt_prop_predict_new_subjects.stan")
...
data <- read_csv("zero_into_depot_1cmt_linear/Data/zero_into_depot_1cmt_prop.csv", na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
  group_by(ID) %>% 
...
est_ind <- if(stan_data$want_auc_cmax){
  preds_df %>%
    spread_draws(c(CL, VC, KA, DUR, t_half, 
                   auc_ss, c_max, t_max)[ID]) %>%
    median_qi() %>% 
    select(ID, CL, VC, KA, DUR, t_half,
           auc_ss, c_max, t_max) %>% 
  ...
  preds_df %>%
    spread_draws(c(CL, VC, KA, DUR)[ID]) %>% 
    median_qi() %>% 
