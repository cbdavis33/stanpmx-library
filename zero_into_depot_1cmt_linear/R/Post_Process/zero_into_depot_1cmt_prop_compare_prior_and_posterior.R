...
stan_data <- jsonlite::read_json(
  "zero_into_depot_1cmt_linear/Stan/Fits/Stan_Data/prop.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$prior_only <- stan_data$no_gq_predictions <- 1

model <- cmdstan_model(
  "zero_into_depot_1cmt_linear/Stan/Fit/zero_into_depot_1cmt_prop.stan",
...
(target_comparison_tv <- draws_all_df %>% 
    ...
    mutate(variable = factor(variable, 
                             levels = str_c("TV", c("CL", "VC", "KA", "DUR")))) %>% 
    ggplot() +
...
(target_comparison_omega <- draws_all_df %>% 
    ...
    mutate(variable = factor(variable, 
                             levels = str_c("omega_", c("cl", "vc", "ka", "dur"))),
...
(target_comparison_cor <- draws_all_df %>% 
    ...
    mutate(variable = factor(variable, 
                             levels = c("cor_cl_vc", "cor_cl_ka", "cor_cl_dur", 
                                        "cor_vc_ka", "cor_vc_dur", 
                                        "cor_ka_dur")),
