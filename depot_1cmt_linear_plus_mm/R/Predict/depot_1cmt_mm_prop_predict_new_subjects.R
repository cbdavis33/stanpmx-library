...
# For this example, let's simulate 50 mg, 100 mg, 200 mg, 400 mg, 800 mg, 1600 mg 
dosing_data <- mrgsolve::expand.ev(addl = 6, ii = 24, cmt = 1, 
                                   amt = c(50, 100, 200, 400, 800, 1600), 
                                   tinf = 0, evid = 1, mdv = 1) %>%
  as_tibble() %>% 
  mutate(ss = 0) %>% 
...
model <- cmdstan_model(
  "depot_1cmt_mm/Stan/Predict/depot_1cmt_mm_prop_predict_new_subjects.stan")
...
regimens <- str_c(c(50, 100, 200, 400, 800, 1600), " mg")
...
data <- read_csv("depot_1cmt_mm/Data/depot_1cmt_mm_prop.csv", na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
  group_by(ID) %>% 
