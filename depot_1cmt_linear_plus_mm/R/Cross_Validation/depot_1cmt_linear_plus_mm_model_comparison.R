...
filename_base <- "depot_1cmt_linear_plus_mm/Stan/Fits"

fit_ppa <- read_rds(file.path(filename_base, "depot_1cmt_linear_plus_mm_ppa.rds"))
...
stan_data <- jsonlite::read_json(
  "depot_1cmt_linear_plus_mm/Stan/Fits/Stan_Data/ppa.json") %>% 
  map(function(x) if(is.list(x)) as_vector(x) else x)

stan_data$n_sim <- 1000

model_ppa <- cmdstan_model(
  "depot_1cmt_linear_plus_mm/Stan/Cross_Validation/depot_1cmt_linear_plus_mm_ppa_logo_cv_sim.stan")

model_prop <- cmdstan_model(
  "depot_1cmt_linear_plus_mm/Stan/Cross_Validation/depot_1cmt_linear_plus_mm_prop_logo_cv_sim.stan")

model_exp <- cmdstan_model(
  "depot_1cmt_linear_plus_mm/Stan/Cross_Validation/depot_1cmt_linear_plus_mm_exp_logo_cv_sim.stan")
