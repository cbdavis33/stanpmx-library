...
# fileConn <- file("depot_1cmt_linear_plus_mm/output.txt")
...
mcmc_combo(fit$draws(parameters_to_examine %>% str_subset("omega")),
           combo = c("dens_overlay", "trace", "violin"))
mcmc_combo(fit$draws(c("sigma_p", "sigma_a", "cor_p_a")),
           combo = c("dens_overlay", "trace", "violin"))
...
mcmc_rank_hist(fit$draws(c("sigma_p", "sigma_a", "cor_p_a")))
...
mcmc_rank_overlay(fit$draws(c("sigma_p", "sigma_a", "cor_p_a")))
...
# mcmc_rank_ecdf(fit$draws(c("sigma_p", "sigma_a", "cor_p_a")))
...
mcmc_pairs(fit$draws(c("omega_km", str_c("KM[", 1:9, "]"))),
           transform = list(omega_km = "log"),
           diag_fun = "dens",
           off_diag_fun = "hex")


mcmc_pairs(fit$draws(c("omega_cl", "CL[1]", "omega_vc", "VC[1]", 
                       "omega_vmax", "VMAX[1]", "omega_km", "KM[1]", 
                       "omega_ka", "KA[1]")),
