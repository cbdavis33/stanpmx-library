...
model <- cmdstan_model(
  "depot_1cmt_linear_plus_mm/Stan/Predict/depot_1cmt_linear_plus_mm_prop_predict_observed_subjects.stan")
...
## VPCs. Also see ../VPC/depot_1cmt_linear_plus_mm_prop_vpc.R for an alternate but equivalent
## way to do VPCs
...
## NPDEs. Also see ../NPDE/depot_1cmt_linear_plus_mm_prop_npde.R for an alternate but 
...
blah <- draws_df %>%
  gather_draws(CL[ID], VC[ID], VMAX[ID], KM[ID], KA[ID], 
               TVCL, TVVC, TVVMAX, TVKM, TVKA) %>%
  ungroup() %>%
...
ggplot() +
  ...
  scale_fill_manual(name = "Population Parameter",
                    values = c("red", "blue", "green", "purple", "magenta")) +
  guides(color = "none") +
