...
mcmc_combo(fit$draws(c("TVCL", "TVVC", "TVVMAX", "TVKM", "TVKA")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_cl", "omega_vc", "omega_vmax", "omega_km", 
                       "omega_ka")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("sigma_p", "sigma_a")),
           combo = c("dens_overlay", "trace"))

mcmc_rank_hist(fit$draws(c("TVCL", "TVVC", "TVVMAX", "TVKM", "TVKA")))
mcmc_rank_hist(fit$draws(c("omega_cl", "omega_vc", "omega_vmax", "omega_km", 
                           "omega_ka")))
...
blah <- draws_df %>%
  gather_draws(c(CL, VC, VMAX, KM, KA)[ID], 
...
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, VMAX, KM, KA, 
                 eta_cl, eta_vc, eta_vmax, eta_km, eta_ka)[ID]) %>%
  mean_qi() %>% 
  select(ID, CL, VC, VMAX, KM, KA,
         eta_cl, eta_vc, eta_vmax, eta_km, eta_ka) %>% 
  mutate(ID = factor(ID))
...
(est_ind %>% 
    ggplot(aes(x = eta_vmax, y = eta_km)) + 
    geom_point() +
    theme_bw() + 
