...
// Ggeneral ODE solution (rk45, bdf, or adams) using Torsten (user's choice)
  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      if(solver == 1){
        ...
        x_ipred[subj_start[j]:subj_end[j],] =
          pmx_solve_adams(depot_1cmt_linear_plus_mm_ode,
                          n_cmt,
                          time[subj_start[j]:subj_end[j]],
                          amt[subj_start[j]:subj_end[j]],
                          rate[subj_start[j]:subj_end[j]],
...
data{
  ...
  int<lower = 1, upper = 3> solver; // 1 = rk45, 2 = bdf, 3 = adams
...
transformed parameters{
...

  {
    ...
    row_vector[n_random] typical_values = to_row_vector({TVCL, TVVC, TVVMAX, 
                                                         TVKM, TVKA});
