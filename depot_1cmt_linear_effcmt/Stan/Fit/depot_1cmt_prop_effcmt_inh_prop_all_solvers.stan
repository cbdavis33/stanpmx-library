  ...
  real partial_sum_lpmf(array[] int seq_subj, int start, int end,
    ...
    for(n in 1:N){            // loop over subjects in this slice
      ...
      if(solver == 1){
        ...
        x_ipred[subj_start[j]:subj_end[j],] =
          pmx_solve_bdf(depot_1cmt_effcmt_ode,
                        n_cmt + n_cmt_pd,
                        time[subj_start[j]:subj_end[j]],
                        amt[subj_start[j]:subj_end[j]],
                        rate[subj_start[j]:subj_end[j]],
        ...
        x_ipred[subj_start[j]:subj_end[j],] =
          pmx_solve_onecpt_bdf(depot_1cmt_effcmt_ode_coupled,
                               n_cmt_pd,
                               time[subj_start[j]:subj_end[j]],
                               amt[subj_start[j]:subj_end[j]],
                               rate[subj_start[j]:subj_end[j]],
...
data{
  ...
  int<lower = 1, upper = 5> solver; // 1 = mat-exp, 2 = General rk45, 3 = Coupled rk45, 4 = General bdf, 5 = Coupled bdf
