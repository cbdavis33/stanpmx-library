...
## Read in fit
fit <- read_rds("depot_1cmt_linear_effcmt/Stan/Fits/depot_1cmt_prop_effcmt_inh_prop.rds")
...
mcmc_combo(fit$draws(c("TVE0", "TVKE0", "TVEC50")),
           combo = c("dens_overlay", "trace"))
mcmc_combo(fit$draws(c("omega_e0", "omega_ke0", "omega_ec50")),
           combo = c("dens_overlay", "trace"))
...
mcmc_rank_hist(fit$draws(c("TVE0", "TVKE0", "TVEC50")))
mcmc_rank_hist(fit$draws(c("omega_e0", "omega_ke0", "omega_ec50")))
...
est_ind <- draws_df %>%
  spread_draws(c(CL, VC, KA, E0, KE0, EC50, 
                 eta_cl, eta_vc, eta_ka,
                 eta_e0, eta_ke0, eta_ec50)[ID]) %>% 
  mean_qi() %>% 
  select(ID, CL, VC, KA, E0, KE0, EC50,
         eta_cl, eta_vc, eta_ka, eta_e0, eta_ke0, eta_ec50) %>% 
  mutate(ID = factor(ID))
