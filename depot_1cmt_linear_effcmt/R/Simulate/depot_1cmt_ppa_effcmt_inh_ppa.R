...
R_pd[1, 3] <- R_pd[3, 1] <- 0.7 # Put in some correlation between E0 and EC50
...
stan_data <- list(n_subjects = n_subjects,
                  ...
                  solver = 1) # Matrix-exponential = 1, General ODE = 2, Coupled ODE = 3

model <- cmdstan_model(
  "depot_1cmt_linear_effcmt/Stan/Simulate/depot_1cmt_ppa_effcmt_inh_ppa.stan") 
...
params_ind <- simulated_data$draws(c("CL", "VC", "KA",
                                     "E0", "KE0", "EC50")) %>% 
  spread_draws(c(CL, VC, KA, E0, KE0, EC50)[i]) %>% 
  inner_join(dosing_data %>% 
               mutate(i = 1:n()),
             by = "i") %>% 
  ungroup() %>%
  select(ID, CL, VC, KA, E0, KE0, EC50)
...
data %>%
  select(-IPRED) %>% 
  write_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_ppa_effcmt_inh_ppa.csv", na = ".")
  # write_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_prop_effcmt_inh_prop.csv", na = ".")

params_ind %>%
  write_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_ppa_effcmt_inh_ppa_params_ind.csv")
  # write_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_prop_effcmt_inh_prop_params_ind.csv")
