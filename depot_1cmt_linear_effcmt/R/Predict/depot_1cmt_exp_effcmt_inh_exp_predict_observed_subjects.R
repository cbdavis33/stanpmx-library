...
nonmem_data <- read_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_exp_effcmt_inh_exp.csv",
                        na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
...
model <- cmdstan_model(
  "depot_1cmt_linear_effcmt/Stan/Predict/depot_1cmt_exp_effcmt_inh_exp_predict_observed_subjects.stan")
