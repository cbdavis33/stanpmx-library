...
model <- cmdstan_model(
  "depot_1cmt_linear_effcmt/Stan/Predict/depot_1cmt_ppa_effcmt_inh_ppa_predict_new_subjects.stan")
...
data <- read_csv("depot_1cmt_linear_effcmt/Data/depot_1cmt_ppa_effcmt_inh_ppa.csv", 
                 na = ".") %>% 
  rename_all(tolower) %>% 
  rename(ID = "id",
         DV = "dv") %>% 
